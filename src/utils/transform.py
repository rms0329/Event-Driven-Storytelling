"""
All code snippet in this file are borrowed from Pytorch3D and Kornia:
...
# borrowed from kornia (https://github.com/kornia/kornia)
def quaternion_pow(quaternions: torch.Tensor, t: Union[torch.Tensor, float], eps: float = 1e-12):
    """Return the power of a quaternion raised to exponent t."""
    if isinstance(t, torch.Tensor):
        assert t.shape == quaternions.shape[:-1], "Exponent must have the same batch shape as the quaternions."
        t = t.unsqueeze(-1)

    polar_angle = (quaternions[..., 0] / quaternions.norm(p=2, dim=-1).clamp(min=eps)).acos()
    theta = polar_angle[..., None]
    vec = quaternions[..., 1:]
    vec_norm = vec.norm(dim=-1, keepdim=True).clamp(min=eps)
    n = torch.where(vec_norm != 0, vec / vec_norm, vec * 0)
    w = (t * theta).cos()
    xyz = (t * theta).sin() * n
...
def quaternion_slerp(
    quaternions0: torch.Tensor,
    quaternions1: torch.Tensor,
    t: Union[torch.Tensor, float],
):
    """Return the spherical linear interpolation between two quaternions."""

    q0 = F.normalize(quaternions0, p=2.0, dim=-1)
    q1 = F.normalize(quaternions1, p=2.0, dim=-1)
    return quaternion_multiply(
        q0,
        quaternion_pow(quaternion_multiply(quaternion_invert(q0), q1), t),
    )


# https://math.stackexchange.com/a/2355070
def quaternion_relative_rotation(rot_from, rot_to):
    return quaternion_multiply(rot_to, quaternion_invert(rot_from))


def quaternion_angle_between(rot_from, rot_to):
    rel_rot = quaternion_relative_rotation(rot_from, rot_to)
    return 2 * torch.acos(rel_rot[..., 0:1].clamp(-1, 1))


def quaternion_aligning_rotation(v0, v1):
    bs = v0.shape[:-1]
    v0 /= torch.norm(v0, dim=-1, keepdim=True)
    v1 /= torch.norm(v1, dim=-1, keepdim=True)

    axis = torch.cross(v0.float(), v1.float(), dim=-1)
    dot = (v0 * v1).sum(dim=-1)
    quat = torch.zeros(bs + (4,), device=v0.device, dtype=v0.dtype)
    quat[..., 1:] = axis
    quat[..., 0] = 1 + dot
